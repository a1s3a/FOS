...
void idt_init(void){
	...
	gdt[GD_TSS >> 3] = SEG16(STS_T32A, (uint32) (&ts), sizeof(struct Taskstate), 0);
	gdt[GD_TSS >> 3].sd_s = 0;
	// Load the TSS
	ltr(GD_TSS);
	// Load the IDT
...
static void trap_dispatch(struct Trapframe *tf){
	...
	}else if(tf->tf_trapno == T_DBLFLT)	panic("double fault!!");
	else if (tf->tf_trapno == IRQ0_Clock)clock_interrupt_handler() ;
	else{
...
//		env_page_ws_print(curenv);*/
		if(isBufferingEnabled())__page_fault_handler_with_buffering(faulted_env, fault_va);
		else page_fault_handler(faulted_env, fault_va);
...
void table_fault_handler(struct Env * curenv, uint32 fault_va){
	...
	uint32* ptr_table;
	if(USE_KHEAP)ptr_table = create_page_table(curenv->env_page_directory, (uint32)fault_va);
	else __static_cpt(curenv->env_page_directory, (uint32)fault_va, &ptr_table);
}
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va){}
//Handle the page fault
uint32 LRU(struct Env *curenv){
	uint32 Minimum = env_page_ws_get_time_stamp(curenv, 0);
	uint32 Ret = 0;
	uint32 Ind = 1;
	while (Ind < curenv->page_WS_max_size){
		uint32 Tmp = env_page_ws_get_time_stamp(curenv, Ind);
		if (Tmp < Minimum){
			Minimum = Tmp;
			Ret = Ind;
		}
...
uint32 Clock(struct Env *curenv){
	while (1){
		uint32 VA = curenv->ptr_pageWorkingSet[ClockLastInd].virtual_address;
		uint32 Perms = pt_get_page_permissions(curenv, VA);
		++ClockLastInd;
		ClockLastInd %= curenv->page_WS_max_size;
		if (!(Perms & PERM_USED))return (ClockLastInd + curenv->page_WS_max_size - 1) % curenv->page_WS_max_size;
		pt_set_page_permissions(curenv, VA, 0, PERM_USED);
	}
}
uint32 Try2(struct Env*);
uint32 Try1(struct Env *curenv){
	uint32 tmp = ClockLastInd;
	while (1){
		uint32 VA = curenv->ptr_pageWorkingSet[tmp].virtual_address;
		uint32 Perms = pt_get_page_permissions(curenv, VA);
		++tmp;
		tmp %= curenv->page_WS_max_size;
		if (!(Perms & PERM_USED) && !(Perms & PERM_MODIFIED)){
			ClockLastInd = tmp;
			return (ClockLastInd + curenv->page_WS_max_size - 1) % curenv->page_WS_max_size;
		}
		pt_set_page_permissions(curenv, VA, 0, PERM_USED);
		if (tmp == ClockLastInd)return curenv->page_WS_max_size;
	}
}
...
uint32 Fifo(struct Env *curenv){
	return 0;
}
uint32 ModifiedClock(struct Env *curenv){
	while (1){
		uint32 Ret = Try1(curenv);
		if (Ret != curenv->page_WS_max_size)return Ret;
		Ret = Try2(curenv);
		if (Ret != curenv->page_WS_max_size)return Ret;
	}
}
void Replacement(struct Env *curenv, uint32 fault_va, uint32 Victim){
	...
	uint32 Perms = pt_get_page_permissions(curenv, va);
	if ((Perms & PERM_MODIFIED))pf_update_env_page(curenv, (void*)va, ptr_frame_info);
	unmap_frame(curenv->env_page_directory, (void*)va);
	env_page_ws_clear_entry(curenv, Victim);
}
void Placement(struct Env *curenv, uint32 fault_va, uint32 Victim){
	struct Frame_Info *ptr_frame_info = NULL;
	if (allocate_frame(&ptr_frame_info) == E_NO_MEM)panic("No memory for page in memory");
	...
	if (Ret == E_PAGE_NOT_EXIST_IN_PF){
		if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP){
			if (pf_add_empty_env_page(curenv, fault_va, 0) == E_NO_PAGE_FILE_SPACE)panic("No space for page in page file");
...
void page_fault_handler(struct Env * curenv, uint32 fault_va){
	...
	if (env_page_ws_get_size(curenv) == curenv->page_WS_max_size){
		if (isPageReplacmentAlgorithmCLOCK())Victim = Clock(curenv);
		else if (isPageReplacmentAlgorithmModifiedCLOCK())Victim = ModifiedClock(curenv);
		else if (isPageReplacmentAlgorithmFIFO())Victim = Fifo(curenv);
		else Victim = LRU(curenv);
		Replacement(curenv, fault_va, Victim);
	}else Victim = curenv->page_last_WS_index;
	Placement(curenv, fault_va, Victim);
}
