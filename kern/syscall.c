...
static int sys_env_destroy(int32 envid){
	...
	if (e == curenv)cprintf("[%08x] exiting gracefully\n", curenv->env_id);
	else cprintf("[%08x] destroying %08x\n", curenv->env_id, e->env_id);
	//2015
...
static int __sys_allocate_page(void *va, int perm){
	...
	if (r == E_NO_MEM)return r ;
	//check virtual address to be paged_aligned and < USER_TOP
	if ((uint32)va >= USER_TOP || (uint32)va % PAGE_SIZE != 0)return E_INVAL;
	//check permissions to be appropriate
	if ((perm & (~PERM_AVAILABLE & ~PERM_WRITEABLE)) != (PERM_USER))return E_INVAL;
	...
	if(USE_KHEAP){
		...
		ptr_frame_info->references -= 1;
	}else memset(STATIC_KERNEL_VIRTUAL_ADDRESS(physical_address), 0, PAGE_SIZE);
...
void sys_freeMem(uint32 virtual_address, uint32 size){
	if(isBufferingEnabled())__freeMem_with_buffering(curenv, virtual_address, size);
	else freeMem(curenv, virtual_address, size);
	return;
}
...
void sys_clearFFL(){
	...
	for (; i < size ; i++)allocate_frame(&ptr_tmp_FI) ;
}
...
int sys_create_env(char* programName, unsigned int page_WS_size){
	...
	if(env == NULL)return E_ENV_CREATION_ERROR;
	//2015
	sched_new_env(env);
	return env->env_id;
}
...
void chktst(uint32 n){
	...
	else panic("Error!! test fails at final");
}
//2015
