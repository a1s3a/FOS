...
void initialize_kernel_VM(){
	...
	}else boot_map_range(ptr_page_directory, KERNEL_BASE, 0xFFFFFFFF - KERNEL_BASE, 0, PERM_WRITEABLE) ;
...
void* boot_allocate_space(uint32 size, uint32 align){
	...
	if (ptr_free_mem == 0)ptr_free_mem = end_of_kernel;
	/* Your code here:
...
void initialize_paging(){
	...
	for (i = PHYS_IO_MEM/PAGE_SIZE ; i < PHYS_EXTENDED_MEM/PAGE_SIZE; i++)frames_info[i].references = 1;
...
void decrement_references(struct Frame_Info* ptr_frame_info){
	if (--(ptr_frame_info->references) == 0)free_frame(ptr_frame_info);
}
...
int get_page_table(uint32 *ptr_page_directory, const void *virtual_address, uint32 **ptr_page_table){
	...
	else *ptr_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(page_directory_entry)) ;
	if ( (page_directory_entry & PERM_PRESENT) == PERM_PRESENT)return TABLE_IN_MEMORY;
...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address){
	...
	uint32 Ret = (uint32)kmalloc(PAGE_SIZE);
	uint32 PA =  kheap_physical_address(Ret);
...
int map_frame(uint32 *ptr_page_directory, struct Frame_Info *ptr_frame_info, void *virtual_address, int perm){
	...
	if( get_page_table(ptr_page_directory, virtual_address, &ptr_page_table) == TABLE_NOT_EXIST){
		...
		=============================================================================================*/
		if(USE_KHEAP)ptr_page_table = create_page_table(ptr_page_directory, (uint32)virtual_address);
		else __static_cpt(ptr_page_directory, (uint32)virtual_address, &ptr_page_table);
	}
	uint32 page_table_entry = ptr_page_table[PTX(virtual_address)];
	/*OLD WRONG SOLUTION
	if( EXTRACT_ADDRESS(page_table_entry) != physical_address){
		if( page_table_entry != 0)unmap_frame(ptr_page_directory , virtual_address);
	...
	if ((page_table_entry & PERM_PRESENT) == PERM_PRESENT){
		...
		if (EXTRACT_ADDRESS(page_table_entry) == physical_address)return 0;
		//on another pa, then unmap it
		else unmap_frame(ptr_page_directory , virtual_address);
	}
	ptr_frame_info->references++;
	ptr_page_table[PTX(virtual_address)] = CONSTRUCT_ENTRY(physical_address , perm | PERM_PRESENT);
	return 0;
...
void unmap_frame(uint32 *ptr_page_directory, void *virtual_address){
	...
	if( ptr_frame_info != 0 ){
		if (ptr_frame_info->isBuffered && !CHECK_IF_KERNEL_ADDRESS((uint32)virtual_address))cprintf("Freeing BUFFERED frame at va %x!!!\n", virtual_address) ;
...
uint32 calculate_required_frames(uint32* ptr_page_directory, uint32 start_virtual_address, uint32 size){
	...
	for(; current_virtual_address < (start_virtual_address+size); current_virtual_address+= PAGE_SIZE*1024){
		...
		if(ptr_page_table == 0)(number_of_tables)++;
	}
	...
	for(; current_virtual_address < (start_virtual_address+size); current_virtual_address+= PAGE_SIZE){
		uint32 *ptr_page_table;
		if (get_frame_info(ptr_page_directory, (void*) current_virtual_address, &ptr_page_table) == 0)(number_of_pages)++;
	}
...
struct freeFramesCounters calculate_available_frames(){
	...
	while (slowPtr && fastPtr) {
		...
		if (fastPtr == NULL)break; // since fastPtr is NULL we reached the tail
	...
	LIST_FOREACH(ptr, &free_frame_list){
		if (ptr->isBuffered)totalFreeBuffered++ ;
		else totalFreeUnBuffered++ ;
	}
	LIST_FOREACH(ptr, &modified_frame_list)totalModified++ ;
...
void env_page_ws_print(struct Env *curenv){
	...
	for(i=0; i< (curenv->page_WS_max_size); i++ ){
		if (curenv->ptr_pageWorkingSet[i].empty){
			...
			if(i==curenv->page_last_WS_index )cprintf("		<--");
			cprintf("\n");
			continue;
		}
...
void env_table_ws_print(struct Env *curenv){
	...
	for(i=0; i< __TWS_MAX_SIZE; i++ ){
		if (curenv->__ptr_tws[i].empty){
			cprintf("EMPTY LOCATION");
			if(i==curenv->table_last_WS_index )cprintf("		<--");
			cprintf("\n");
			continue;
		}
...
void bufferList_add_page(struct Linked_List* bufferList,struct Frame_Info *ptr_frame_info){
	/*cprintf("inserting frame_info %x, mod_first = %x, mod_last = %x\n", ptr_frame_info , LIST_FIRST(&modified_frame_list), LIST_LAST(&modified_frame_list));
		...
		struct Frame_Info *ptr;
		if (ptr_frame_info == (struct Frame_Info*) 0xf02ebd78)cprintf("the frame is buffered into MODIFIED list *******************************\n");
		LIST_FOREACH(ptr, &free_frame_list){
		...
		struct Frame_Info *ptr;
		if (ptr_frame_info == (struct Frame_Info*) 0xf02ebd78)cprintf("the frame is buffered into FREE list *******************************\n");
...
inline void pt_clear_page_table_entry(struct Env* ptr_env, uint32 virtual_address){
	...
	uint32 	page_directory_entry = ptr_pgdir[PDX(virtual_address)] ;
	if ((page_directory_entry & PERM_PRESENT) == PERM_PRESENT){
		if(USE_KHEAP && !CHECK_IF_KERNEL_ADDRESS(virtual_address))ptr_page_table = (uint32*)kheap_virtual_address(EXTRACT_ADDRESS(page_directory_entry)) ;
		else ptr_page_table = STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(page_directory_entry)) ;
		ptr_page_table[PTX(virtual_address)] = 0 ;
	}else if (page_directory_entry != 0){ //the table exists but not in main mem, so it must be in sec mem
		/*cprintf("Warning %d: pt_is_page_modified() is called while the page table is on disk!!\n", ++cnt);
		...
		ptr_page_table[PTX(virtual_address)] = 0 ;
		__pf_write_env_table(ptr_env, virtual_address, (void*) ptr_temp_page);
	}else panic("function pt_clear_page_table_entry() called with invalid virtual address. The corresponding page table doesn't exist\n") ;
	tlb_invalidate((void *)NULL, (void *)virtual_address);
}
...
inline void add_frame_to_storage(uint32* frames_storage, struct Frame_Info* ptr_frame_info, uint32 index){
	...
	if(r == TABLE_NOT_EXIST){
		if(USE_KHEAP)ptr_page_table = create_page_table(frames_storage, (uint32)va);
		else __static_cpt(frames_storage, (uint32)va, &ptr_page_table);
	}
...
inline void clear_frames_storage(uint32* frames_storage){
	...
	if (frames_storage[i] != 0){
		if(USE_KHEAP)kfree((void*)kheap_virtual_address(EXTRACT_ADDRESS(frames_storage[i])));
		else free_frame(to_frame_info(EXTRACT_ADDRESS(frames_storage[i])));
		frames_storage[i] = 0;
	}
}
/********************************************************************************* //
