...
void fos_scheduler(void){
	...
	// Run the special idle environment when nothing else is runnable.
		...
		while (1)run_command_prompt(NULL);
	}
}
/*void
fos_scheduler(void){
	...
	if (isRunMultiTask()){
		if (current_env == NULL)cprintf("current_env = NULL\n") ;
		else cprintf("current_env = %s\n", current_env->prog_name) ;
		if(current_env == NULL)current_env = LIST_FIRST(&env_ready_queue);
		else current_env = LIST_NEXT(current_env);
		while(!env_found){
			if (current_env == NULL)cprintf("current_env = NULL\n") ;
			else cprintf("current_env = %s\n", current_env->prog_name) ;
			if(current_env == sched_last_run_env){
				if(current_env != NULL && current_env->env_status==ENV_READY)env_found=1;
				break;
			}
			if(current_env != NULL){
			...
			}else current_env = LIST_FIRST(&env_ready_queue);
		}
		//we have a runnable env, so update the sched pointer and run it
	}
	//cprintf("Scheduler\n");
...
void sched_insert_ready(struct Env* env){
	if(env != NULL){
		...
		if(sched_next != NULL)LIST_INSERT_BEFORE(&env_ready_queue, sched_next, env);
		else{
...
void sched_print_all(){
	...
	if (!LIST_EMPTY(&env_new_queue)){
		...
		LIST_FOREACH(ptr_env, &env_new_queue)cprintf("	[%d] %s\n", ptr_env->env_id, ptr_env->prog_name);
	}else cprintf("\nNo processes in NEW queue\n");
	cprintf("================================================\n");
	if (!LIST_EMPTY(&env_ready_queue)){
		cprintf("The processes in READY queue are:\n");
		LIST_FOREACH(ptr_env, &env_ready_queue)cprintf("	[%d] %s\n", ptr_env->env_id, ptr_env->prog_name);
	}else cprintf("No processes in READY queue\n");
	cprintf("================================================\n");
	if (!LIST_EMPTY(&env_exit_queue)){
		cprintf("The processes in EXIT queue are:\n");
		LIST_FOREACH(ptr_env, &env_exit_queue)cprintf("	[%d] %s\n", ptr_env->env_id, ptr_env->prog_name);
	}else cprintf("No processes in EXIT queue\n");
}
void sched_run_all(){
	...
	if (scheduler_status == SCH_STOPPED)fos_scheduler();
}
void sched_kill_all(){
	struct Env* ptr_env ;
	if (!LIST_EMPTY(&env_new_queue)){
	...
	}else cprintf("No processes in NEW queue\n");
	cprintf("================================================\n");
	if (!LIST_EMPTY(&env_ready_queue)){
...
void sched_exit_env(uint32 envId){
	...
	if(ptr_env->env_id == envId){
		...
		if (curenv->env_id == envId)fos_scheduler();
		return;
	}
}
/*2015*/
...
void clock_interrupt_handler(){
	if(isPageReplacmentAlgorithmLRU())update_WS_time_stamps();
...
/*void on_clock_update_WS_time_stamps(){
	...
	LIST_FOREACH(curr_env_ptr, &env_ready_queue){
		...
		}
		{
			...
			//if((curr_env_ptr->pageFaultsCounter-old_pf_counter) > 0)//cprintf("[%s]: last clock # of PFs  = %d, timenow = %d\n", curr_env_ptr->prog_name ,curr_env_ptr->pageFaultsCounter-old_pf_counter, time);
	...
	if(curr_env_ptr != NULL){
		{
			...
			if( curr_env_ptr->ptr_pageWorkingSet[i].empty != 1){
				...
				if (perm & PERM_USED){
					...
					pt_set_page_permissions(curr_env_ptr, page_va, 0 , PERM_USED) ;
				}else curr_env_ptr->ptr_pageWorkingSet[i].time_stamp = (oldTimeStamp>>2);
			}
		}
		{
			int t ;
			...
			if( curr_env_ptr->__ptr_tws[t].empty != 1){
				...
				}else curr_env_ptr->__ptr_tws[t].time_stamp = (oldTimeStamp>>2);
			}
		}
	}
}
