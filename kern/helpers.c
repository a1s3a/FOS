...
void detect_memory(){
	...
	}else if (size_of_extended_mem)maxpa = PHYS_EXTENDED_MEM + size_of_extended_mem;
	else maxpa = size_of_extended_mem;
...
void check_boot_pgdir(){
	...
	for (i = 0; i < NPDENTRIES; i++) {
		...
		break;
		default:
			if (i >= PDX(KERNEL_BASE))assert(ptr_page_directory[i]);
			else assert(ptr_page_directory[i] == 0);
			break;
		}
	}
...
uint32 check_va2pa(uint32 *ptr_page_directory, uint32 va){
	...
	if (!(*dirEntry & PERM_PRESENT))return ~0;
	p = (uint32*) STATIC_KERNEL_VIRTUAL_ADDRESS(EXTRACT_ADDRESS(*dirEntry));
	//LOG_VARS("ptr to page table  = %x", p);
	if (!(p[PTX(va)] & PERM_PRESENT))return ~0;
...
/*void page_check(){
	...
	assert(EXTRACT_ADDRESS(ptr_page_directory[0]) == to_physical_address(pp0));
	if(USE_KHEAP)kfree((void*)kheap_virtual_address(EXTRACT_ADDRESS(ptr_page_directory[0])));
	else{
...
void turn_on_paging(){
	...
	//ptr_page_directory[0] = ptr_page_directory[PDX(KERNEL_BASE)];*/
	{
		...
		int j = 0;
		for(; i< PDX((uint32)ptr_free_mem); ++i, ++j)ptr_page_directory[j] = ptr_page_directory[i];
	}
	// Install page table.
	...
	ptr_page_directory[0] = 0;*/
	{
		int i = PDX(KERNEL_BASE);
		int j = 0;
		for(; i< PDX((uint32)ptr_free_mem); ++i, ++j)ptr_page_directory[j] = 0;
	}
